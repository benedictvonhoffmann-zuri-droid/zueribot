"""
Zürich POI Connector
- Raw data from OpenStreetMap via overpass.osm.ch
"""
...
def _resolve_location(query, lat=None, lon=None):
    """Resolve location from query using Nominatim. Returns (poi_query, lat, lon)."""
    if lat and lon:
...
def search_poi(query, lat=None, lon=None, radius=2000, limit=10):
    """Search POIs via OpenStreetMap Overpass API."""
    try:
        poi_query, search_lat, search_lon = _resolve_location(query, lat, lon)
        ...
        if not results:
            return {
                "success": False,
                "data": None,
                "source": {"name": "OpenStreetMap", "type": "community"},
                "error": f"No results found for '{query}' within {radius}m"
            }
        ...
        return {"success": False, "data": None, "source": {"name": "OpenStreetMap", "type": "community"}, "error": str(e)}